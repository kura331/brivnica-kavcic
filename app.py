...
# 1. KONFIGURACIJA IN STIL (Črna tema in črni gumbi)
...
# 2. VARNA POVEZAVA S POPRAVKOM ZA PEM
@st.cache_resource
def povezi():
    try:
        # Preberemo vse iz Secrets
        info = dict(st.secrets["gcp_service_account"])
        # TA VRSTICA POPRAVI PEM NAPAKO:
        info["private_key"] = info["private_key"].replace("\\n", "\n")
        
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        creds = Credentials.from_service_account_info(info, scopes=scope)
        # Povezava na tabelo BarberBooking
...
if st.session_state.korak == 1:
    ...
    if st.button("NADALJUJ"): # Gumb nadaljuj
        if ime and tel:
